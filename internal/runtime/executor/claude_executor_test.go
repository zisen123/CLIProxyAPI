...
func TestApplyClaudeHeaders_UsesConfiguredBaselineFingerprint(t *testing.T) {
	...
	applyClaudeHeaders(req, auth, "key-baseline", false, nil, cfg, "claude-opus-4-8")
...
func TestApplyClaudeHeaders_TracksHighestClaudeCLIFingerprint(t *testing.T) {
	...
	applyClaudeHeaders(firstReq, auth, "key-upgrade", false, nil, cfg, "claude-opus-4-8")
	...
	applyClaudeHeaders(thirdPartyReq, auth, "key-upgrade", false, nil, cfg, "claude-opus-4-8")
	...
	applyClaudeHeaders(higherReq, auth, "key-upgrade", false, nil, cfg, "claude-opus-4-8")
	...
	applyClaudeHeaders(lowerReq, auth, "key-upgrade", false, nil, cfg, "claude-opus-4-8")
...
func TestApplyClaudeHeaders_DoesNotDowngradeConfiguredBaselineOnFirstClaudeClient(t *testing.T) {
	...
	applyClaudeHeaders(olderClaudeReq, auth, "key-baseline-floor", false, nil, cfg, "claude-opus-4-8")
	...
	applyClaudeHeaders(newerClaudeReq, auth, "key-baseline-floor", false, nil, cfg, "claude-opus-4-8")
...
func TestApplyClaudeHeaders_UpgradesCachedSoftwareFingerprintWhenBaselineAdvances(t *testing.T) {
	...
	applyClaudeHeaders(officialReq, auth, "key-baseline-reload", false, nil, oldCfg, "claude-opus-4-8")
	...
	applyClaudeHeaders(thirdPartyReq, auth, "key-baseline-reload", false, nil, newCfg, "claude-opus-4-8")
...
func TestApplyClaudeHeaders_LearnsOfficialFingerprintAfterCustomBaselineFallback(t *testing.T) {
	...
	applyClaudeHeaders(thirdPartyReq, auth, "key-custom-baseline-learning", false, nil, cfg, "claude-opus-4-8")
	...
	applyClaudeHeaders(officialReq, auth, "key-custom-baseline-learning", false, nil, cfg, "claude-opus-4-8")
	...
	applyClaudeHeaders(postLearningThirdPartyReq, auth, "key-custom-baseline-learning", false, nil, cfg, "claude-opus-4-8")
...
func TestApplyClaudeHeaders_ThirdPartyBaselineThenOfficialUpgradeKeepsPinnedPlatform(t *testing.T) {
	...
	applyClaudeHeaders(thirdPartyReq, auth, "key-third-party-then-official", false, nil, cfg, "claude-opus-4-8")
	...
	applyClaudeHeaders(officialReq, auth, "key-third-party-then-official", false, nil, cfg, "claude-opus-4-8")
	assertClaudeFingerprint(t, officialReq.Header, "claude-cli/2.1.77 (external, cli)", "0.87.0", "v24.8.0", "MacOS", "arm64")
}
...
func TestApplyClaudeHeaders_DisableDeviceProfileStabilization(t *testing.T) {
	...
	applyClaudeHeaders(firstReq, auth, "key-disable-stability", false, nil, cfg, "claude-opus-4-8")
	...
	applyClaudeHeaders(thirdPartyReq, auth, "key-disable-stability", false, nil, cfg, "claude-opus-4-8")
	...
	applyClaudeHeaders(lowerReq, auth, "key-disable-stability", false, nil, cfg, "claude-opus-4-8")
...
func TestApplyClaudeHeaders_LegacyModePreservesConfiguredUserAgentOverrideForClaudeClients(t *testing.T) {
	...
	applyClaudeHeaders(req, auth, "key-legacy-ua-override", false, nil, cfg, "claude-opus-4-8")
...
func TestApplyClaudeHeaders_LegacyModeFallsBackToRuntimeOSArchWhenMissing(t *testing.T) {
	...
	applyClaudeHeaders(req, auth, "key-legacy-runtime-os-arch", false, nil, cfg, "claude-opus-4-8")
...
func TestApplyClaudeHeaders_UnsetStabilizationAlsoUsesLegacyRuntimeOSArchFallback(t *testing.T) {
	...
	applyClaudeHeaders(req, auth, "key-unset-runtime-os-arch", false, nil, cfg, "claude-opus-4-8")
...
func TestApplyClaudeHeaders_StripsRedactThinkingForBedrockBackedModel(t *testing.T) {
	resetClaudeDeviceProfileCache()

	auth := &cliproxyauth.Auth{
		ID:         "auth-bedrock",
		Attributes: map[string]string{"api_key": "key-bedrock"},
	}
	...
	cases := []struct {
		...
		{"aws suffix", "claude-opus-4-8-aws", true},
		{"anthropic prefix", "anthropic.claude-opus-4-8", true},
		{"first-party model", "claude-opus-4-8", false},
		{"cc variant", "claude-opus-4-8-cc", false},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			req := newClaudeHeaderTestRequest(t, nil)
			if err := applyClaudeHeaders(req, auth, "key-bedrock", false, nil, nil, tc.baseModel); err != nil {
				t.Fatalf("applyClaudeHeaders returned error: %v", err)
			}
			got := req.Header.Get("Anthropic-Beta")
			hasFlag := strings.Contains(got, "redact-thinking-2026-02-12")
			if tc.wantStripped && hasFlag {
				t.Fatalf("model %q: Anthropic-Beta should strip redact-thinking-2026-02-12, got %q", tc.baseModel, got)
			}
			if !tc.wantStripped && !hasFlag {
				t.Fatalf("model %q: Anthropic-Beta should keep redact-thinking-2026-02-12, got %q", tc.baseModel, got)
			}
		})
	}
}
